...
{
    public static class MassimoProductMapper
    {
        public static List<ProductToListDto> Map(Root root)
        {
            ...
            {
                Console.WriteLine("MASSIMO MAPPER: Root və ya products null-dur");
            ...
            foreach (var product in root.products)
            {
                try
                {
                    ...
                    {
                        Console.WriteLine($"MASSIMO MAPPER: {product?.name ?? "Unknown"} - Detail yoxdur");
                        continue;
                    }
                    ...
                    {
                        Console.WriteLine($"MASSIMO MAPPER: {product.name} - Rəng yoxdur");
                        continue;
                    }
                    ...
                    for (int i = 0; i < detail.colors.Count; i++)
                    {
                        ...
                        {
                            Console.WriteLine($"MASSIMO MAPPER: {product.name} - {color?.name} üçün ölçü yoxdur");
                            continue;
                        }
                        ...
                        {
                            Console.WriteLine($"MASSIMO MAPPER: Qiymət xətası {product.name} - {ex.Message}");
                        }
                        ...
                        {
                            Console.WriteLine($"MASSIMO MAPPER: Şəkil xətası {product.name} - {color.name} - {ex.Message}");
                        }
                ...
                {
                    Console.WriteLine($"MASSIMO MAPPER XƏTA: {product?.name ?? "Unknown"} - {ex.Message}");
                    continue;
                }
            }

            Console.WriteLine($"MASSIMO MAPPER: {mappedProducts.Count} məhsul map edildi");
            return mappedProducts;
        }
    }
}