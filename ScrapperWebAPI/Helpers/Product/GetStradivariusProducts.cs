...
public static class GetStradivariusProducts
{
    ...
    static GetStradivariusProducts()
    {
        ...
        _client.DefaultRequestHeaders.Add("Referer", "https://www.stradivarius.com/");
        _client.DefaultRequestHeaders.Add("Origin", "https://www.stradivarius.com");
        ...
        _jsonSettings = new JsonSerializerSettings
        {
            ...
            Error = (sender, args) =>
            {
                ...
                Console.WriteLine($"STRADIVARIUS JSON xeta: Path: {path} | Error: {error.Message}");
    ...
    public static async Task<List<ProductToListDto>> FetchProductsAsync(List<string> urls)
    {
        ...
        {
            Console.WriteLine("STRADIVARIUS: Bos URL list");
            return new List<ProductToListDto>();
        }

        Console.WriteLine($"STRADIVARIUS: Basladi - {urls.Count} URL");
        ...
        var tasks = urls.Select(async (url, index) =>
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"STRADIVARIUS ERROR: URL {index + 1} - {ex.Message}");
            }
            finally
            {
                _semaphore.Release();
    ...
    private static async Task<List<ProductToListDto>> ProcessSingleUrl(string url, int current, int total)
    {
        try
        {
            ...
            {
                Console.WriteLine($"STRADIVARIUS: {current}/{total} - Bos cavab");
                return null;
            }
            ...
            {
                Console.WriteLine($"STRADIVARIUS: {current}/{total} - JSON xetasi: {jsonEx.Message.Substring(0, Math.Min(80, jsonEx.Message.Length))}");
                return null;
            }

            if (data?.products == null || data.products.Count == 0)
            {
                Console.WriteLine($"STRADIVARIUS: {current}/{total} - Mehsul yoxdur");
                return null;
            }

            var products = StradivariusProductMapper.Map(data);

            if (products != null && products.Count > 0)
            {
                Console.WriteLine($"STRADIVARIUS: {current}/{total} - {products.Count} mehsul");
        ...
        {
            Console.WriteLine($"STRADIVARIUS: {current}/{total} - {ex.GetType().Name}: {ex.Message}");
            return null;
        }
    }
    ...
    private static async Task<string> GetWithRetry(string url, int maxRetries = 3)
    {
        ...
        Console.WriteLine($"STRADIVARIUS RETRY: {maxRetries} cehd ugursuz - {lastException?.Message}");
        return null;
    }
    ...
    private static async Task SendProductsToAPI(List<ProductToListDto> products, string sourceUrl)
    {
        if (products == null || products.Count == 0)
        {
            Console.WriteLine("STRADIVARIUS API: Məhsul yoxdur");
            return;
        }
        ...
        Console.WriteLine($"STRADIVARIUS API: {totalProducts} məhsul göndərməyə başlayır...");
        ...
        try
        {
            ...
            for (int i = 0; i < products.Count; i++)
            {
                ...
                try
                {
                    ...
                    for (int attempt = 1; attempt <= 3 && !success; attempt++)
                    {
                        try
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                if ((i + 1) % 10 == 0)
                                {
                                    Console.WriteLine($"STRADIVARIUS API: [{i + 1}/{totalProducts}] göndərildi (Uğurlu={sentCount}, Xəta={errorCount})");
                                }
                            }
                            else
                            {
                                Console.WriteLine($"STRADIVARIUS API xəta: {product.Name} - HTTP {response.StatusCode} (Cəhd {attempt}/3)");
                        ...
                        {
                            Console.WriteLine($"STRADIVARIUS API timeout: {product.Name} (Cəhd {attempt}/3)");
                        ...
                        {
                            Console.WriteLine($"STRADIVARIUS API HTTP xəta: {product.Name} - {ex.Message} (Cəhd {attempt}/3)");
                    ...
                    {
                        errorCount++;
                        Console.WriteLine($"STRADIVARIUS API: {product.Name} ATILDI (bütün cəhdlər uğursuz)");
                    }
                ...
                {
                    errorCount++;
                    Console.WriteLine($"STRADIVARIUS API xəta: {product.Name} - {ex.Message} - DAVAM EDİR");
                    continue;
                }
            }
        }
        ...
        {
            Console.WriteLine($"STRADIVARIUS API kritik xəta: {ex.Message} - Göndərilən: {sentCount}/{totalProducts}");
        }

        Console.WriteLine($"STRADIVARIUS API NƏTİCƏ: Cəmi={totalProducts}, Göndərilən={sentCount}, Xəta={errorCount}");
    }
    ...
    private static async Task WarmupConnection()
    {
        try
        {
            await _client.GetAsync("https://www.stradivarius.com/az/");
