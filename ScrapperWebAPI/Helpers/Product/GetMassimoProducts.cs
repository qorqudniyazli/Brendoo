...
namespace ScrapperWebAPI.Helpers.Product
{
    public static class GetMassimoProducts
    {
        ...
        static GetMassimoProducts()
        {
            ...
            _client.DefaultRequestHeaders.Add("Referer", "https://www.massimodutti.com/");
            _client.DefaultRequestHeaders.Add("Origin", "https://www.massimodutti.com");
            ...
            _jsonSettings = new JsonSerializerSettings
            {
                ...
                Error = (sender, args) =>
                {
                    ...
                    Console.WriteLine($"MASSIMO JSON xeta: Path: {path} | Error: {error.Message}");
        ...
        public static async Task<List<ProductToListDto>> FetchProductsAsync(List<string> urls)
        {
            ...
            {
                Console.WriteLine("MASSIMO: Bos URL list");
                return new List<ProductToListDto>();
            }

            Console.WriteLine($"MASSIMO: Basladi - {urls.Count} URL");
            ...
            var tasks = urls.Select(async (url, index) =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine($"MASSIMO ERROR: URL {index + 1} - {ex.Message}");
                }
                finally
                {
                    _semaphore.Release();
        ...
        private static async Task<List<ProductToListDto>> ProcessSingleUrl(string url, int current, int total)
        {
            try
            {
                ...
                {
                    Console.WriteLine($"MASSIMO: {current}/{total} - Bos cavab");
                    return null;
                }
                ...
                {
                    Console.WriteLine($"MASSIMO: {current}/{total} - JSON xetasi: {jsonEx.Message.Substring(0, Math.Min(80, jsonEx.Message.Length))}");
                    return null;
                }

                if (data?.products == null || data.products.Count == 0)
                {
                    Console.WriteLine($"MASSIMO: {current}/{total} - Mehsul yoxdur");
                    return null;
                }
                ...
                if (products != null && products.Count > 0)
                {
                    Console.WriteLine($"MASSIMO: {current}/{total} - {products.Count} mehsul");
            ...
            {
                Console.WriteLine($"MASSIMO: {current}/{total} - {ex.GetType().Name}: {ex.Message}");
                return null;
            }
        }
        ...
        private static async Task<string> GetWithRetry(string url, int maxRetries = 3)
        {
            ...
            Console.WriteLine($"MASSIMO RETRY: {maxRetries} cehd ugursuz - {lastException?.Message}");
            return null;
        }
        ...
        private static async Task SendProductsToAPI(List<ProductToListDto> products, string sourceUrl)
        {
            if (products == null || products.Count == 0)
            {
                Console.WriteLine("MASSIMO API: Məhsul yoxdur, API çağırılmadı");
                return;
            }
                ...
                for (int i = 0; i < products.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        var productData = new
                        {
                            ...
                            brand = product.Brand ?? "Massimo Dutti",
                        ...
                        for (int attempt = 1; attempt <= 3 && !success; attempt++)
                        {
                            try
                            {
                                ...
                                if (response.IsSuccessStatusCode)
                                {
                                    ...
                                    if ((i + 1) % 10 == 0)
                                    {
                                        Console.WriteLine($"MASSIMO API: [{i + 1}/{totalProducts}] göndərildi (Uğurlu={sentCount}, Xəta={errorCount})");
                                    }
                                }
                                else
                                {
                                    Console.WriteLine($"MASSIMO API xəta: {product.Name} - HTTP {response.StatusCode} (Cəhd {attempt}/3)");
                            ...
                            {
                                Console.WriteLine($"MASSIMO API timeout: {product.Name} (Cəhd {attempt}/3)");
                            ...
                            {
                                Console.WriteLine($"MASSIMO API HTTP xəta: {product.Name} - {ex.Message} (Cəhd {attempt}/3)");
                        ...
                        {
                            errorCount++;
                            Console.WriteLine($"MASSIMO API: {product.Name} ATILDI (bütün cəhdlər uğursuz)");
                        }
                    ...
                    {
                        errorCount++;
                        Console.WriteLine($"MASSIMO API xəta: {product.Name} - {ex.Message} - DAVAM EDİR");
                        continue;
                    }
                }
            }
            ...
            {
                Console.WriteLine($"MASSIMO API kritik xəta: {ex.Message} - Göndərilən: {sentCount}/{totalProducts}");
            }

            Console.WriteLine($"MASSIMO API NƏTİCƏ: Cəmi={totalProducts}, Göndərilən={sentCount}, Xəta={errorCount}");
        }
        ...
        private static async Task WarmupConnection()
        {
            try
            {
                await _client.GetAsync("https://www.massimodutti.com/az/");
