...
public static class GetPullAndBearProducts
{
    ...
    static GetPullAndBearProducts()
    {
        ...
        _client.DefaultRequestHeaders.Add("Referer", "https://www.pullandbear.com/az");
        _client.DefaultRequestHeaders.Add("Origin", "https://www.pullandbear.com/az");
        ...
        _jsonSettings = new JsonSerializerSettings
        {
            ...
            Error = (sender, args) =>
            {
                ...
                Console.WriteLine($"Pull And Bear JSON xeta: Path: {path} | Error: {error.Message}");
    ...
    public static async Task<List<ProductToListDto>> FetchProductsAsync(List<string> urls)
    {
        ...
        {
            Console.WriteLine("Pull And Bear: Bos URL list");
            return new List<ProductToListDto>();
        }

        Console.WriteLine($"Pull And Bear: Basladi - {urls.Count} URL");
        ...
        var tasks = urls.Select(async (url, index) =>
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"Pull And Bear ERROR: URL {index + 1} - {ex.Message}");
            }
            finally
            {
                _semaphore.Release();
    ...
    private static async Task<List<ProductToListDto>> ProcessSingleUrl(string url, int current, int total)
    {
        try
        {
            ...
            {
                Console.WriteLine($"Pull And Bear: {current}/{total} - Bos cavab");
                return null;
            }
            ...
            {
                Console.WriteLine($"Pull And Bear: {current}/{total} - JSON xetasi: {jsonEx.Message.Substring(0, Math.Min(80, jsonEx.Message.Length))}");
                return null;
            }

            if (data?.products == null || data.products.Count == 0)
            {
                Console.WriteLine($"Pull And Bear: {current}/{total} - Mehsul yoxdur");
                return null;
            }

            var products = PullAndBearProductMapper.Map(data);

            if (products != null && products.Count > 0)
            {
                Console.WriteLine($"Pull And Bear: {current}/{total} - {products.Count} mehsul");
        ...
        {
            Console.WriteLine($"Pull And Bear: {current}/{total} - {ex.GetType().Name}: {ex.Message}");
            return null;
        }
    }
    ...
    private static async Task<string> GetWithRetry(string url, int maxRetries = 3)
    {
        ...
        Console.WriteLine($"Pull And Bear RETRY: {maxRetries} cehd ugursuz - {lastException?.Message}");
        return null;
    }
    ...
    private static async Task SendProductsToAPI(List<ProductToListDto> products, string sourceUrl)
    {
        if (products == null || products.Count == 0)
        {
            Console.WriteLine("Pull And Bear API: Məhsul yoxdur, API çağırılmadı");
            return;
        }
            ...
            for (int i = 0; i < products.Count; i++)
            {
                ...
                try
                {
                    ...
                    var productData = new
                    {
                        ...
                        brand = product.Brand ?? "Pull And Bear",
                    ...
                    for (int attempt = 1; attempt <= 3 && !success; attempt++)
                    {
                        try
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                if ((i + 1) % 10 == 0)
                                {
                                    Console.WriteLine($"Pull And Bear API: [{i + 1}/{totalProducts}] göndərildi (Uğurlu={sentCount}, Xəta={errorCount})");
                                }
                            }
                            else
                            {
                                Console.WriteLine($"Pull And Bear API xəta: {product.Name} - HTTP {response.StatusCode} (Cəhd {attempt}/3)");
                        ...
                        {
                            Console.WriteLine($"Pull And Bear API timeout: {product.Name} (Cəhd {attempt}/3)");
                        ...
                        {
                            Console.WriteLine($"Pull And Bear API HTTP xəta: {product.Name} - {ex.Message} (Cəhd {attempt}/3)");
                    ...
                    {
                        errorCount++;
                        Console.WriteLine($"Pull And Bear API: {product.Name} ATILDI (bütün cəhdlər uğursuz)");
                    }
                ...
                {
                    errorCount++;
                    Console.WriteLine($"Pull And Bear API xəta: {product.Name} - {ex.Message} - DAVAM EDİR");
                    continue;
                }
            }
        }
        ...
        {
            Console.WriteLine($"Pull And Bear API kritik xəta: {ex.Message} - Göndərilən: {sentCount}/{totalProducts}");
        }

        Console.WriteLine($"Pull And Bear API NƏTİCƏ: Cəmi={totalProducts}, Göndərilən={sentCount}, Xəta={errorCount}");
    }
    ...
    private static async Task WarmupConnection()
    {
        try
        {
            await _client.GetAsync("https://www.pullandbear.com/az/");
